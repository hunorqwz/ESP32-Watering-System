...
async function runTest() {
  ...
  let disabledWaterLevelSensor = false;
  let originalFlowRate = 4.0;
  let testPassed = true;

  try {
    // Temporarily rename water_level sensor type to disabled_water_level
    const waterSensorCheck = await client.query("SELECT id FROM sensor_configs WHERE type = 'water_level'");
    if (waterSensorCheck.rows.length > 0) {
      console.log('Temporarily disabling water_level sensor config to enforce flow-rate calculation...');
      await client.query("UPDATE sensor_configs SET type = 'disabled_water_level' WHERE type = 'water_level'");
      disabledWaterLevelSensor = true;
    }
    ...
    originalFlowRate = origRes.rows[0]?.flow_rate_lpm !== null && origRes.rows[0]?.flow_rate_lpm !== undefined ? parseFloat(origRes.rows[0].flow_rate_lpm) : 4.0;
    ...
    if (log.duration_seconds >= 14 && log.duration_seconds <= 18) {
      console.log('Result: SUCCESS (Duration is accurate)');
    } else {
      console.error('Result: FAILURE (Duration is outside expected range of 14-18s)');
      testPassed = false;
    }
    ...
    try {
      ...
      if (disabledWaterLevelSensor) {
        console.log('Restoring water_level sensor config...');
        await client.query("UPDATE sensor_configs SET type = 'water_level' WHERE type = 'disabled_water_level'");
      }
